...
public class DRAFT extends ActiveRouter {
	 ...
	 */
	/*
	 * # ##EXAMPLE## DRAFT protocol settings
Group.router = DRAFT
...
下面几个变量与脚本中的这个几个参数对应
	...
	public void checkLocalCommunity(Connection con) {
		...
		if(this.neighbourSet.get(peer) >= this.familiarThreshold) {//判断是否大于阈值，如果是则
			...
			// 4.1 if the node is still not in the local community, give it another chance with this secondary promotion mechanism
			if(!this.localClusterCache.containsKey(peer))
			{
				//把节点peer及它的簇树都加入进来 这样做的话节点链接的簇树会有重复的节点，作用是什么？保证peer节点还有机会再次加入localcluster
	...
	public void update() {
		...
		for(Connection c : getConnections()) {
			...
			DRAFT peerC = (DRAFT) c.getOtherNode(getHost()).getRouter();
                        ...
                        //初始化neighbourSet                           
                        /*
                            我的想法：如果接触时间超过某一个值，则将peer次数加1；否则视为没有相遇；（如何获取相遇时间？）已验证：改变衰减率的值对结果影响效果不明显
                            */
			...
			if(this.neighbourSet.get(peer) >= this.familiarThreshold) {//如果次数大于阈值，则
				checkLocalCommunity(c);//达到要求则加入到本地通信组localClusterCache
                 ...
                 //       System.out.println("******"+c.getConnectiontime()+"********");
                //        }
		}
